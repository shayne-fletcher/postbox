...
- `gradient()`: helper function for computing forward-mode gradients of f: ℝⁿ → ℝ
- `Var<T>`: tape-based reverse-mode AD (backpropagation)
- `reverse_diff()`: helper function for single-variable reverse-mode differentiation
- `reverse_gradient()`: helper function for reverse-mode gradients of f: ℝⁿ → ℝ
